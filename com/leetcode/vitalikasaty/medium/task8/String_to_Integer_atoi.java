package com.leetcode.vitalikasaty.medium.task8;
...
public class String_to_Integer_atoi {
	...
	public static void main(String[] args) {
		...
		System.out.println(solution.myAtoi("   642req"));
	}
}
...
class Solution {
	public int myAtoi(String s) {
		...
		String resultString = "";
		int plusMinus = 1;
		int startPosNumberInString = 0;
		int answer = 0;

		for (int i = 0; i < s.length(); i++) {
			if (s.charAt(i) == '-') {
				plusMinus = -1;
				startPosNumberInString = i + 1;
				break;
			} else if (s.charAt(i) == '+') {
				startPosNumberInString = i + 1;
				break;
			} else if (s.charAt(i) >= 48 && s.charAt(i) <= 57) {
				startPosNumberInString = i;
				break;
			} else if (s.charAt(i) != ' ') {
				return 0;
			}
		...
		for (int i = startPosNumberInString; i < s.length(); i++) {
		...
		try {
			answer = Integer.parseInt(resultString) * plusMinus;
