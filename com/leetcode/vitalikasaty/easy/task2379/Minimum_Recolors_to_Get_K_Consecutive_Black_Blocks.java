package com.leetcode.vitalikasaty.easy.task2379;
...
class Solution {
	public int minimumRecolors(String blocks, int k) {
		...
		for (int i = 0; i <= blocks.length() - k; i++) {
			String KIBlock = blocks.substring(i, i + k);
			int countRecolor = (int) KIBlock.chars().filter(c -> c == 'W').count();
			if (countRecolor < result) {
				result = countRecolor;
			}
		}
